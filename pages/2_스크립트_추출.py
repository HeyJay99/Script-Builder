...
with extract_col:
    extract_btn = st.button(
        "스크립트 추출", type="primary", use_container_width=True, disabled=not media_file
    )

if extract_btn and media_file:
    st.info("⚠️ 추출 중 다른 페이지로 이동하면 작업이 중단됩니다.")
    st.session_state.pop("extracted_script", None)  # 이전 결과 초기화 (#5 버그 수정)
    try:
